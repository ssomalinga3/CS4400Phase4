...
class ProcedureRunnerApp:
    def __init__(self, master):
        ...
        style.configure('Header.TLabel', font=('Helvetica', 20, 'bold'), background='#0288D1', foreground='white') # header label
        style.configure('Section.TLabel', font=('Helvetica', 12), background='#E0F7FA') # section labels
        style.configure('Flight.TButton', font=('Helvetica', 10, 'bold'), background='#0288D1', foreground='white') # buttons
        style.map('Flight.TButton', background=[('active', '#0277BD')])
        style.configure('TCombobox', fieldbackground='white') # dropdown
        style.configure('TLabel', background='#E0F7FA')
        style.configure('Treeview', rowheight=25) # table rows
        ...
        self.combo = ttk.Combobox(select_frame, textvariable=self.selection,
                                  values=list(procedures.keys()) + list(views.keys()), state='readonly') # dropdown menu
        self.combo.grid(row=0, column=1, sticky='we', padx=5)
        self.combo.bind('<<ComboboxSelected>>', self.build_params) # when user picks something
        ...
        ttk.Button(master, text='Run', style='Flight.TButton', command=self.run).pack(pady=5) # run button
    ...
    def build_params(self, event):
        ...
        key = self.selection.get() # gets current selection 
        param_list = procedures.get(key, []) # looks up needed parameters

        if not param_list:
            ttk.Label(self.params_frame, text='(No parameters)', style='Section.TLabel').pack(anchor='w')
            return
        
        # for every parameter needed, creates a row with input field & expected type
        for name, typ in param_list:
            ...
            self.entries.append((name, typ, ent)) # stores user input
        ...
        for name, typ, ent in self.entries:
            ...
            if typ == 'int':
                try:
                    ...
                    return
            args.append(val) # build arg list
        try:
            # connect to the database
            conn = get_connection()
            cur = conn.cursor()

            # run the procedure (if its a procedure)
            if key in procedures:
                ...
                self.tree['columns'] = []
            else: # if its a view, select that
